...
function mkdirfix (name, opts, cb) {
  mkdirp(name, { fs: opts.fs }, function (err, made) {
    if (!err && made && opts.own) {
      chownr(made, opts.uid, opts.gid, function (err) {
        if (err) return cb(err)
        if (opts.mode !== undefined && opts.fs.chmod) {
          return opts.fs.chmod(name, opts.mode, cb)
        }
        cb()
      })
    } else {
      if (!err && made && opts.mode !== undefined && opts.fs.chmod) {
        return opts.fs.chmod(name, opts.mode, cb)
      }
      cb(err)
    }
